...
func setPSK(psk string, wgh *wg.WireGuardHandler, cfg *config.Config, logPrefix string) error {
	if cfg.UsePQC() {
...
func listenIds(cfg *config.Config, wgh *wg.WireGuardHandler, kmsServer *kms.KMSHandler, skipPush chan bool, done chan bool) {
	...
	for r := range result {
		...
		err = setPSK(key.GetKey(), wgh, cfg, "<-- BACKUP:")
		if err != nil {
			log.Println(err.Error())
		}
	}
...
func pushIds(cfg *config.Config, wgh *wg.WireGuardHandler, kmsServer *kms.KMSHandler, skipPush chan bool) {
	...
	for {
		...
		default:
			...
			err = setPSK(key.GetKey(), wgh, cfg, "--> MASTER:")
			if err != nil {
				log.Println(err.Error())
			}
		}
